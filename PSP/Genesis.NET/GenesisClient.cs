...
namespace Genesis.Net
{
    internal class GenesisClient : IGenesisClient
    {
        ...
        public MemoryStream Execute(byte[] raw) {
            var webRequest = CreateRawWebRequest(raw);
        ...
        private WebRequest CreateWebRequest(Request request) {
            ...
            // Console.WriteLine("Genesis Request: {0}", dump);
        ...
        private WebRequest CreateRawWebRequest(byte[] request) {
            string url = ComposeUrl("gate", "process", true);
            var webRequest = WebRequest.Create(url);
            webRequest.Headers["Authorization"] = configuration.Authorization;
            webRequest.Method = "POST";
            webRequest.ContentType = "text/xml";
        ...
        {
            string url = null;
            if(configuration.Environment.ToUrlName() != String.Empty)
                url = String.Format("https://{0}.{1}.{2}/{3}/", configuration.Environment.ToUrlName(), subDomain, configuration.getEndpointURL(), apiPath);
            else
                url = String.Format("https://{0}.{1}/{2}/", subDomain, configuration.getEndpointURL(), apiPath);
